...
using VoxelTensor = pyskip::Tensor<3, int32_t>;
...
auto compute_adjacency_mask(const VoxelTensor& tensor) {
  ...
  auto sl = [&](int x_0, int x_1, int y_0, int y_1, int z_0, int z_1) {
    ...
    y_1 = y_1 <= 0 ? h + y_1 : y_1;
    z_1 = z_1 <= 0 ? d + z_1 : z_1;
    return pyskip::TensorSlice<3>(
        {{x_0, x_1, 1}, {y_0, y_1, 1}, {z_0, z_1, 1}});
  };

  auto update = [](auto& tensor, auto slice, auto input) {
    auto merged = tensor.get(slice).array() | input;
    tensor.set(slice, pyskip::make_tensor<3>(slice.shape(), std::move(merged)));
  };

  auto empty = pyskip::make_tensor<3>(
      tensor.shape(), pyskip::splat<int>(tensor.array() == 0, 0xFFFFFFFF, 0));
  auto adjacencies = pyskip::make_tensor<3>(tensor.shape(), 0);
