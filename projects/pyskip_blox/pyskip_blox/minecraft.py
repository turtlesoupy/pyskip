...
@dataclass
class PySkipMinecraftChunk:
    coord: Tuple[int, int, int]
    tensor: pyskip.Tensor
...
class NumpyMinecraftChunk:
    ...
    def to_pyskip(self):
        return PySkipMinecraftChunk(
            self.coord,
            pyskip.Tensor.from_numpy(self.tensor),
        )


class PySkipMinecraftLevel:
    def __init__(
        self,
        chunk_list: List[PySkipMinecraftChunk],
    ...
    def load(self, path):
        with gzip.open(path, "rb") as f:
            ret = pickle.load(f)
            ret.chunk_list = [e.to_pyskip() for e in ret.chunk_list]
            return ret

    def num_nonzero_voxels(self):
        return sum((len(chunk.tensor) - pyskip.reduce.sum((chunk.tensor == 0).to(int))) for chunk in self.chunk_list)
    ...
    def sparse_compression_ratio(self):
        ...
        for chunk in self.chunk_list:
            ...
            sparse_size += 2 * (len(chunk.tensor) - pyskip.reduce.sum((chunk.tensor == 0).to(int)))
    ...
    def megatensor(self):
        dim_1, dim_2, dim_3 = self._xyz_to_pyskip_col(self.dense_dimensions(), self.column_order)
        megatensor = pyskip.Tensor(shape=(dim_1, dim_2, dim_3), dtype=int)
        for chunk in self.chunk_list:
            start_x = chunk.coord[0] - self.bbox[0][0]
            start_y = chunk.coord[1] - self.bbox[1][0]
            start_z = chunk.coord[2] - self.bbox[2][0]

            xyz_tensor_shape = self._pyskip_col_to_xyz(chunk.tensor.shape, self.column_order)
            ...
            try:
                megatensor[self._xyz_to_pyskip_col(rng, self.column_order)] = chunk.tensor
            except ValueError:
                print(rng)
                raise
        return megatensor
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i, chunk in enumerate(tqdm(world.iter_chunks(), total=world.chunk_count())):
                ...
                if as_numpy:
                    ...
                    chunk_list.append(cls.chunk_to_pyskip((x, y, z), chunk, column_order=column_order))
    ...
    def xyz_to_pyskip_col(self, item):
        return self.__class__._xyz_to_pyskip_col(item, self.column_order)

    @classmethod
    def _xyz_to_pyskip_col(cls, item, column_order):
    ...
    def _xyz_to_numpy_col(cls, item, column_order):
        return tuple(reversed(cls._xyz_to_pyskip_col(item, column_order)))

    def pyskip_col_to_xyz(self, item):
        return self.__class__._pyskip_col_to_xyz(item, self.column_order)

    @classmethod
    def _pyskip_col_to_xyz(cls, item, column_order):
    ...
    @classmethod
    def chunk_to_pyskip(cls, coord, chunk, column_order=(0, 1, 2)):
        numpy_chunk = cls.chunk_to_numpy(coord, chunk, column_order)
        tensor = pyskip.Tensor.from_numpy(numpy_chunk.tensor)
        return PySkipMinecraftChunk(numpy_chunk.coord, tensor)

    @classmethod
    def map_for_chunk(cls, chunk, column_order):
        pixels = b""
        max_x, max_y, max_z = cls._pyskip_col_to_xyz(chunk.tensor.shape, column_order)
