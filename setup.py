    ...
    def build_extension(self, ext):
        ...
        subprocess.check_call(
            args=platform_args(
                ...
                windows_args=["-Ax64", f"-DCMAKE_LIBRARY_OUTPUT_DIRECTORY_{config.upper()}={output_dir}",],
                posix_args=[f"-DCMAKE_BUILD_TYPE={config}",],
            ),
            cwd=self.build_temp,
            env=env,
        )
        ...
        subprocess.check_call(
            args=["cmake", "--build", ".", "--target", "skimpy_ext", "--config", config], cwd=self.build_temp,
        )
